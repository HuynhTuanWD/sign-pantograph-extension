...
function wbpspane_save_custom_fields()
{
    ...
    foreach ($fields as $field) {
        if (isset($_POST[$field])) {
            update_option($field, esc_html($_POST[$field]));
        }
    }
    echo ("<fieldset>");
...
function wbpspane_wpdocs_save_meta_box($post_id)
{
    if (isset($_POST['wp_bacoor_content'])) {
        update_post_meta($post_id, 'wp_bacoor_content', esc_html($_POST['wp_bacoor_content']));
    }
}
...
function wbpspane_wpdocs_save_meta_box_1($post_id)
{
    if (isset($_POST['wp_bacoor_content_default'])) {
        update_post_meta($post_id, 'wp_bacoor_content_default', esc_html($_POST['wp_bacoor_content_default']));
    }
}
